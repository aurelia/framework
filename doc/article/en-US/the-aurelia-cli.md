...
The CLI sets up the default hosting directory as `/`, but ASP.NET Core's convention is to serve static files from the `/wwwroot` subdirectory. This can result in a `404 Not Found` error with a message saying `Cannot GET /` if you run your Aurelia app from the CLI. To avoid this error and allow the CLI to serve your application, edit the `/aurelia_project/task/run.ts` (or `/aurelia_project/task/run.js`) file and change the `baseDir` property from `.` to `./wwwroot`:
```javascript
...
  server: {
    baseDir: ['./wwwroot'],
...
Similarly, for ASP.NET Core to serve the files in `/wwwroot` generated by Aurelia, you will need to enable file serving in your application's `Startup.cs` file if you have not already done so. In the `Configure(...)` method, add the following lines:
```c#
  // Enable serving of static files, including the default index.html file
  app.UseFileServer();
```

Finally, by default Visual Studio's TypeScript compiler will attempt to compile the Aurelia project files along with your application's actual source files, and this will result in build errors. To resolve these conflicts, it is necessary to exclude the Aurelia project directory from Visual Studio's TypeScript compilation process. Add the following to your project's `tsconfig.json` file:
```javascript
"exclude": [
  ...
  "aurelia_project" // Exclude this folder from TypeScript builds
],
```

If you wish to integrate the CLI's build process with the ASP.NET Core build process, you can add the following to your application's `project.json` file. This assumes that the CLI has been installed with `-g`.
```javascript
"scripts": {
  "precompile": [ "au build" ], // Run 'au build' first whenever the ASP.NET Core project is compiled
  ...
}
```
While full rebuilds like this take time, it is expected that you would normally use the CLI to watch for file changes and provide incremental compilation. This is described further in the following section.
